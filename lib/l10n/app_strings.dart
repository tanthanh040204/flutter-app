...
class AppStrings {
  ...
  String get loginHint => vi
      ? 'Nhập email và mật khẩu đã đăng ký để đăng nhập.'
      : 'Enter your registered email and password to log in.';
  String get registerHint => vi
      ? 'Đăng ký cần họ tên, email, mật khẩu, xác nhận mật khẩu và số điện thoại.'
      : 'Registration requires full name, email, password, password confirmation, and phone number.';
  String get invalidEmail =>
      vi ? 'Vui lòng nhập email hợp lệ.' : 'Please enter a valid email.';
  String get phoneRequired => vi
      ? 'Đăng ký bắt buộc phải có số điện thoại.'
      : 'Phone number is required for registration.';
  String get passwordRequired =>
      vi ? 'Vui lòng nhập mật khẩu.' : 'Please enter your password.';
  String get confirmPasswordRequired =>
      vi ? 'Vui lòng nhập xác nhận mật khẩu.' : 'Please confirm your password.';
  String get passwordMinLength => vi
      ? 'Mật khẩu phải có ít nhất 6 ký tự.'
      : 'Password must be at least 6 characters.';
  String get passwordMismatch => vi
  ...
  String get passwordChanged =>
      vi ? 'Đổi mật khẩu thành công.' : 'Password changed successfully.';

  String get onboardingTitle1 => vi
      ? 'Phương tiện công cộng - Đi bất kỳ đâu'
      : 'Public vehicles - Go anywhere';
  String get onboardingDesc1 => vi
      ? 'Bạn có thể lấy xe tại một trạm, thực hiện chuyến đi và trả xe tại một trạm bất kỳ.'
      : 'Pick up a bike at one station, take your trip, and return it at any station.';
  String get onboardingTitle2 =>
      vi ? 'Quét QR để mở khóa' : 'Scan QR to unlock';
  String get onboardingDesc2 => vi
      ? 'Chỉ cần quét QR trên xe và xác nhận sử dụng nếu số dư của bạn hợp lệ.'
      : 'Scan the QR code on the bike and confirm if your balance is valid.';
  String get onboardingTitle3 => vi ? 'Kết thúc chuyến đi' : 'End your trip';
  String get onboardingDesc3 => vi
      ? 'Đỗ xe tại trạm, khóa xe và xác nhận trả xe trên ứng dụng di động để kết thúc chuyến đi.'
      : 'Park at a station, lock the bike, and confirm return in the mobile app.';
  ...
  String get deposit => vi ? 'Tiền thuê' : 'Deposit';
  String get debt => vi ? 'Tiền nợ' : 'Debt';
  String get topUp => vi ? 'Nạp tiền' : 'Top up';
  String get scanQr => vi ? 'Quét QR' : 'Scan QR';
  String get enterRentalTime =>
      vi ? 'Nhập thời gian thuê' : 'Enter rental time';
  ...
  String get invalidHours =>
      vi ? 'Vui lòng nhập số giờ hợp lệ.' : 'Please enter valid rental hours.';
  String selectedRentalHours(int hours) => vi
      ? 'Thời gian đã chọn: $hours giờ'
      : 'Selected duration: $hours hour(s)';
  String rentalFee(String amount) =>
      vi ? 'Tiền thuê: $amount' : 'Rental fee: $amount';
  String depositFee(String amount) =>
      vi ? 'Tiền cọc: $amount' : 'Deposit: $amount';
  String requiredTotal(String amount) =>
      vi ? 'Tổng cần có: $amount' : 'Required total: $amount';
  String get noRideTitle =>
      vi ? 'Hiện chưa có chuyến đi nào' : 'No active ride';
  String get noRideDesc => vi
      ? 'Bạn có thể nhập số giờ muốn thuê, nạp tiền và quét QR trên xe để bắt đầu sử dụng.'
      : 'Enter rental hours, top up, and scan the bike QR code to start riding.';
  String vehicleInUse(String name) =>
      vi ? '$name đang được sử dụng' : '$name is in use';
  String get remainingTimeDesc => vi
      ? 'Thời gian còn lại được tính theo số giờ bạn đã nhập khi thuê xe.'
      : 'Remaining time is calculated from the hours you entered when renting.';
  String get resume => vi ? 'Tiếp tục' : 'Resume';
  String get pause => vi ? 'Tạm ngưng' : 'Pause';
  String get end => vi ? 'Kết thúc' : 'End';
  String get extendWarning => vi
      ? 'Bạn còn 15p sử dụng xe, vui lòng thuê thêm hoặc trả xe về trạm sớm nhất nhé!'
      : 'You have 15 minutes left. Please extend your ride or return the bike to a station soon.';
  String get returnStationWarning =>
      vi ? 'Vui lòng trả xe về trạm' : 'Please return the bike to a station';

  String get scanQrUnlockTitle =>
      vi ? 'Quét QR mở khóa xe' : 'Scan QR to unlock bike';
  String get invalidQr => vi
      ? 'QR không hợp lệ. App chỉ nhận QR có nội dung bắt đầu bằng "haq-trk-".'
      : 'Invalid QR. The app only accepts QR codes starting with "haq-trk-".';
  String vehicleNameFromCode(String code) => vi ? 'Xe $code' : 'Bike $code';
  String confirmUseVehicle(String vehicleName) => vi
      ? 'Bạn muốn sử dụng $vehicleName?'
      : 'Do you want to use $vehicleName?';
  String confirmRideEnoughBalance({
    required String vehicleId,
    required int hours,
    required String usageFee,
  ...
  String confirmRideNotEnoughBalance({
    required String vehicleId,
    required int hours,
    required String requiredAmount,
  }) {
  ...
  String get bleScanning =>
      vi ? 'Bluetooth: đang tìm xe' : 'BLE: scanning for bike';
  String get bleConnecting =>
      vi ? 'Bluetooth: đang kết nối' : 'BLE: connecting';
  String get bleRelaying =>
      vi ? 'Bluetooth đang truyền dữ liệu' : 'BLE relay active';
  String startedRide(String vehicleName, int hours) => vi
      ? 'Đã bắt đầu thuê $vehicleName trong $hours giờ.'
      : 'Started renting $vehicleName for $hours hour(s).';
  String startRideFailed(String error) => vi
      ? 'Không thể bắt đầu chuyến đi: $error'
      : 'Could not start the ride: $error';

  String get notUsingBike =>
      vi ? 'Bạn chưa sử dụng xe' : 'You are not using a bike';
  String get bikeLockedWaitingData => vi
      ? 'Xe đang khóa. App sẽ nhận dữ liệu khi ESP32 mở khóa.'
      : 'Bike is locked. The app will receive data after ESP32 unlocks.';
  ...
  String get lowBatteryReturnThreshold =>
      vi ? 'Ngưỡng pin nên trả xe' : 'Recommended return battery threshold';

  String get topUpByQr =>
      vi ? 'Chuyển khoản theo QR bên dưới' : 'Transfer using the QR below';
  String get bank => vi ? 'Ngân hàng: UTE Bank' : 'Bank: UTE Bank';
  String get accountNumber =>
      vi ? 'Số tài khoản: 0123456789' : 'Account number: 0123456789';
  String get accountName =>
      vi ? 'Tên tài khoản: CONG TY UTE' : 'Account name: CONG TY UTE';
  String get topUpAmount => vi ? 'Số tiền cần nạp' : 'Top-up amount';
  String transferContent(String value) =>
      vi ? 'Nội dung: $value' : 'Transfer content: $value';
  String get topUpCreated =>
      vi ? 'Đã tạo yêu cầu nạp tiền.' : 'Top-up request created.';
  ...
  String extraHours(int hours) =>
      vi ? 'Thời gian thêm: $hours giờ' : 'Extra time: $hours hour(s)';
  String extraFee(String amount) =>
      vi ? 'Tiền cần trừ thêm: $amount' : 'Extra fee: $amount';
  String get confirmExtend => vi ? 'Xác nhận thuê thêm' : 'Confirm extension';
  String extendedRide(int hours) => vi
      ? 'Đã cộng thêm $hours giờ sử dụng.'
      : 'Added $hours hour(s) to your ride.';

  String get dailyBriefingTitle =>
      vi ? 'Tin giao thông & thời tiết hôm nay' : 'Today traffic & weather';
  String dailyBriefingSubtitle(String date) =>
      vi ? 'Cập nhật mỗi ngày • $date' : 'Updated daily • $date';
  String get loadingDailyNews =>
      vi ? 'Đang tải bản tin mới...' : 'Loading latest updates...';
  String get liveUpdated =>
      vi ? 'Tin mới từ nguồn online' : 'Live from online source';
  String get dailyUpdated =>
      vi ? 'Gợi ý thay đổi theo ngày' : 'Daily rotating tip';
  ...
  String get openWeatherForecast =>
      vi ? 'Dự báo thời tiết' : 'Weather forecast';
  String get noticeOfferTitle =>
      vi ? 'Thông báo & ưu đãi hôm nay' : 'Today notices & offers';
  ...
  String get guideStepLoginTitle =>
      vi ? 'Đăng nhập hoặc đăng ký' : 'Log in or register';
  ...
  String get guideStepTopUpTitle =>
      vi ? 'Nạp tiền vào ví' : 'Top up your wallet';
  ...
  String get guideStepTimeTitle =>
      vi ? 'Chọn thời gian thuê' : 'Choose rental time';
  ...
  String get guideStepRideTitle =>
      vi ? 'Theo dõi chuyến đi' : 'Track your ride';
  ...
  String get guideStepReturnTitle =>
      vi ? 'Trả xe và kết thúc' : 'Return and end ride';
  ...
  String get rideCompletedSuccessfully =>
      vi ? 'Hoàn tất thành công' : 'Completed successfully';
  String get rideEndedWarningTitle =>
      vi ? 'Đã kết thúc chuyến đi (cảnh báo)' : 'Ride ended (warning)';
  String get rideGraceWindowStatus => vi
      ? 'Kết thúc trong thời gian cảnh báo 15 phút'
      : 'Ended within the 15-minute grace window';
  ...
  String get rideEndedViolationTitle =>
      vi ? 'Đã kết thúc chuyến đi (vi phạm)' : 'Ride ended (violation)';
  String get ridePenaltyStatus => vi
      ? 'Vượt quá thời gian cảnh báo 15 phút'
      : 'Exceeded the 15-minute warning window';
  ...
  String get suggestedReturnBatteryThreshold => vi
      ? 'Ngưỡng pin khuyến nghị khi trả xe'
      : 'Suggested return battery threshold';

  String get topUpSuccessful =>
      vi ? 'Nạp tiền thành công' : 'Top-up successful';
  String get topUpFailed => vi ? 'Nạp tiền thất bại' : 'Top-up failed';
  String newBalance(String amount) =>
      vi ? 'Số dư mới: $amount' : 'New balance: $amount';
  String memo(String value) =>
      vi ? 'Nội dung chuyển khoản: $value' : 'Memo: $value';
  String get saving => vi ? 'Đang lưu...' : 'Saving...';

  String vehicleLabel(String id) => vi ? 'Xe $id' : 'Bike $id';
  String get unlockingBike =>
      vi ? 'Đang mở khóa xe...' : 'Unlocking the bike...';
  String pricePerHourAmount(int amount) =>
      vi ? '$amountđ/giờ' : '$amountđ/hour';
  String get pauseDiscountSuffix => vi ? ' (giảm 50%)' : ' (50% off)';
  String overdueText(String value) => vi ? 'Quá hạn $value' : 'Overdue $value';
  String get balanceRunningLowTitle =>
      vi ? 'Số dư sắp hết' : 'Balance running low';
  String get outOfBalanceTitle =>
      vi ? 'Hết số dư — vui lòng trả xe' : 'Out of balance — return the bike';
  String get outOfParkingZoneTitle => vi
      ? 'Xe đang ngoài khu vực trả xe hợp lệ'
      : 'Outside a valid parking zone';
  ...
  String get couldNotStartRide =>
      vi ? 'Không thể bắt đầu chuyến đi' : 'Could not start the ride';
  ...
  String errorDescription(String code) {
    switch (code) {
      ...
      case 'ERR_ACCOUNT_DEBT':
        return vi
            ? 'Tài khoản còn phí chưa thanh toán.'
            : 'Account has outstanding fees.';
      case 'ERR_INSUFFICIENT_BALANCE':
        return vi
            ? 'Số dư không đủ để thuê xe.'
            : 'Balance is insufficient to rent.';
      case 'ERR_BIKE_UNAVAILABLE':
        return vi ? 'Xe hiện không khả dụng.' : 'Vehicle is not available.';
      case 'ERR_BIKE_IN_USE':
        return vi
            ? 'Xe đang được người khác sử dụng.'
            : 'Vehicle is currently in use by another rider.';
      case 'ERR_TIME_LIMIT_WARNING':
        return vi
            ? 'Bạn đã vượt thời gian cho phép, đang trong mức cảnh báo.'
            : 'You are over the allowed time (warning).';
      case 'ERR_TIME_LIMIT_EXCEEDED':
        return vi
            ? 'Đã vượt quá thời gian cho phép và bị tính phí phạt.'
            : 'Time limit exceeded — locked and penalised.';
      case 'ERR_OUT_OF_PARKING_ZONE':
        return vi
            ? 'Xe đang ở ngoài khu vực đỗ hợp lệ.'
            : 'Vehicle is outside a valid parking zone.';
